...
| R6-G05 | Medium | The corrected parent-PID/polling probe passes on API 28: the main process remained alive after the child disappeared, although Android removed the visible task. Run it on the remaining R6 device matrix and document that a fatal fault in the main process remains process-fatal; do not claim the subprocess as a production containment boundary. | Exact PASS or explicit parent-restart/failure details tied to the implementation commit, plus post-display confirmation that the main process remains and the probe process does not; do not retain ephemeral PIDs. | Mobile/native | Before C-007-R6 closure |
